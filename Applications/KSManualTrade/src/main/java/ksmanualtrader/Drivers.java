...
public class Drivers {
    ...
    public final static HashMap<String, KSExchangeDriverManifest> driversInstantiated = new HashMap<>(); // Key: Class full name
    ...
    public static void coreComponent(String pathToJar) throws Exception{
        if (jarsLoaded.contains(pathToJar)) {
            GraphiteProgramLauncher.getJournalingObject().log("WARNING", "Jar file " + pathToJar + " is not loaded because it is already loaded.");
            return;
        }
        ...
        GraphiteProgramLauncher.getJournalingObject().log("INFO", "Loaded JAR: " + jarFile.getName());
    }
